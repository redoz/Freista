...
public static class GeneratorHarness
{
    ...
    private static ImmutableArray<MetadataReference> BuildReferences()
    {
        ...
        refs.Add(MetadataReference.CreateFromFile(typeof(Freista.ScenarioAttribute).Assembly.Location));
        return refs.ToImmutableArray();
    }

    /// <summary>
    /// Compiles generated source as a console app against Freista.Mtp (so the emitted entry point binds
    /// to the real <c>FreistaTestApplication.RunAsync</c> signature) and returns any compile errors.
    /// </summary>
    public static ImmutableArray<Diagnostic> CompileWithMtp(string generatedSource)
    {
        var parseOptions = new CSharpParseOptions(LanguageVersion.Preview);
        var refs = References.Add(
            MetadataReference.CreateFromFile(typeof(Freista.Mtp.FreistaTestApplication).Assembly.Location));
    ...
    public static GeneratorDriver RunDriver(string source, string? path = null, string? generateProgram = "false")
    {
        ...
        var optionsProvider = generateProgram is null
            ...
            {
                ["build_property.FreistaGenerateProgram"] = generateProgram,
            });
    ...
    /// <c>build_property.FreistaGenerateProgram</c> analyzer-config value (the MSBuild property a
    ...
    /// <summary>Runs the analyzer over source and returns just the Freista diagnostics.</summary>
    ...
    public static IReadOnlyList<ScenarioDefinition> Definitions(this GeneratorResult result)
    {
        ...
        var type = result.Assembly!.GetType("Freista.Generated.FreistaGenerated");
        Assert.NotNull(type);
