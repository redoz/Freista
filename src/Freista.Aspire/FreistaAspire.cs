...
/// <summary>
/// Bootstrap for a Freista suite that drives an Aspire application. Call it from your own
/// <c>Program.cs</c> (with <c>&lt;FreistaGenerateProgram&gt;false&lt;/FreistaGenerateProgram&gt;</c>).
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// </remarks>
public static class FreistaAspire
{
...
public static class ScenarioContextAspireExtensions
{
    ...
    /// <see cref="FreistaAspire.RunAsync{TAppHost}"/>.</exception>
    public static DistributedApplication Aspire(this ScenarioContext context)
    {
        ...
        return context.Services?.GetService<DistributedApplication>()
            ?? throw new InvalidOperationException(
                ...
                + "FreistaAspire.RunAsync<TAppHost>(args, ...) from your own Program.cs "
                + "(and set <FreistaGenerateProgram>false</FreistaGenerateProgram>).");
    }
}
