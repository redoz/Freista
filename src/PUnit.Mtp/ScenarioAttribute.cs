...
/// <summary>
/// Marks a static method as a PUnit scenario. The method body is source for the PUnit generator
/// (it is never executed directly); PUnit's own Microsoft.Testing.Platform test framework discovers,
/// runs, and reports each Given/When/Then step as its own MTP test node.
/// </summary>
/// <remarks>
/// <para>
/// This replaces the xUnit-coupled attribute that used to live in <c>PUnit.Xunit</c> (which derived
/// from <c>FactAttribute</c> so xUnit's attribute discovery could find it). PUnit.Mtp owns discovery
/// now, so this is a plain marker attribute with no test-runner coupling. It stays in namespace
/// <c>PUnit</c> so authoring is just <c>using PUnit;</c>.
/// </para>
/// <para>
/// The generator matches it by metadata name (<c>PUnit.ScenarioAttribute</c>) and reads the
...
[SuppressMessage("Design", "CA1019:Define accessors for attribute arguments", Justification = "The display-name positional argument is exposed via the intentionally-named ScenarioDisplayName property (the name the source generator and the original PUnit.Xunit attribute used); the caller-info arguments are surfaced via SourceFilePath/SourceLineNumber.")]
