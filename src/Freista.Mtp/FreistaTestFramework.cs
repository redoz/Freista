// CA1033: the ITestFramework / IDataProducer members are implemented explicitly so Freista's own
...
/// <summary>
/// Freista's own Microsoft.Testing.Platform <see cref="ITestPlatformTestFramework"/>. It owns the
...
/// when Freista rode xUnit as an extension).
/// </summary>
/// <remarks>
/// <para>
/// Modeled on xUnit.net's <c>TestPlatformTestFramework</c> but without xUnit's project / config /
/// reporter / serialization machinery: Freista's <c>ScenarioScheduler</c> already owns execution, so
/// this shell only has to bridge MTP's request pipeline to it.
/// </para>
/// <para>
/// This type is public because the escape-hatch bootstrap (<see cref="FreistaTestApplication.RunAsync"/>)
...
/// </para>
/// </remarks>
public class FreistaTestFramework :
    IExtension, ITestPlatformTestFramework, IDataProducer
{
    /// <summary>Stable extension UID for Freista's MTP test framework.</summary>
    ...
    public FreistaTestFramework(IServiceProvider services) => _services = services;

    /// <summary>Carries the sample-local opt-in <paramref name="simulateTime"/> flag (off in production)
    /// from <see cref="FreistaTestApplication"/>'s <c>RunAsync</c> down to the run loop / scheduler.
    ...
    /// <inheritdoc/>
    public string Version => typeof(FreistaTestFramework).Assembly.GetName().Version?.ToString() ?? "1.0.0";

    /// <inheritdoc/>
    public string DisplayName => "Freista";

    /// <inheritdoc/>
    public string Description => "Freista scenario test framework for Microsoft.Testing.Platform";
    ...
    /// <see cref="TestNodeUpdateMessage"/> per step node (see <see cref="FreistaDiscoverer"/>).
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var methodName in ScenarioRegistry.RegisteredMethods)
        {
            ...
            foreach (var node in FreistaDiscoverer.BuildNodes(definition))
            {
    ...
    /// <summary>
    /// Runs the requested tests for the session via the <see cref="FreistaRunLoop"/>: the filter is
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (bus.Failures.Count > 0)
        {
            ...
            ILogger? logger = _services?.GetLoggerFactory().CreateLogger(typeof(FreistaTestFramework).FullName!);
