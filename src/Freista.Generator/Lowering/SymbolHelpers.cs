...
internal static class SymbolHelpers
{
    ...
    public const string ScenarioContextFullName = "Freista.ScenarioContext";

    /// <summary>Returns the receiver type's name if it implements <c>Freista.IPhase</c> (the built-in
    ...
    public static string? PhaseOf(ExpressionSyntax receiver, SemanticModel model)
    {
        ...
        var isPhase = type.AllInterfaces.Any(i =>
            i.Name == "IPhase"
            && i.ContainingNamespace?.ToDisplayString(NoGlobal) == "Freista");
