...
- **Scope:** new `src/Raun.Aspire` package; new `samples/AspireAppointments/` (AppHost, one
...
Let a Raun suite drive a real Aspire application, with the app started once per run and the
resources it depends on healthy before any scenario executes.

## Boundary: plumbing only

`Raun.Aspire` ships **no phase markers and no `[StepName]` steps**. The framework's premise is
...
using the service registration that landed in `26f3c16e`. Nothing needed a Raun type.
...
5. `RaunTestApplication.RunAsync(args, services: provider, preflight: …)`, where the preflight
...
when the run was not started through `RaunAspire.RunAsync`. It returns the app itself rather than
...
| `AspireAppointments.Tests` | The Raun suite: its own DSL, an explicit `Program.cs`, scenarios |

It demonstrates, deliberately:

- **An explicit `Program.cs`** — the first project in the repo with `RaunGenerateProgram=false`,
  and the shape a `dotnet new raun` template should later scaffold.
...
- **Two identities in one scenario.** A scenario acts as an admin *and* as a patient, because
  ...
  against Raun's own intra-scenario step parallelism.
- **A hand-written typed client**, not Kiota. The DI shape is identical; Kiota would drag an OpenAPI
  document and a codegen step into the sample and start teaching Kiota rather than Raun.

## Testing

`Raun.Aspire` is mostly integration glue, so its tests split by what can be tested without Docker:

| Level | Coverage |
|---|---|
| Unit (`test/Raun.Aspire.Test`) | `AspireRunOptions` accumulates `WaitFor` names and `Services` callbacks across calls; `StartupTimeout` defaults to 5 minutes; `ctx.Aspire()` throws an actionable message when no `DistributedApplication` is registered, and returns it when one is. |
...
- **Cross-process code coverage.** The roadmap's stated hard requirement and its own words call it
  ...
  natural `Raun.Aspire` feature; if it does not, nothing here depends on it. Runs as a spike
  against this sample once it exists.
- **A `dotnet new raun` template** scaffolding the explicit `Program.cs`.
